taskkill /F /IM synergy-tray.exe /T
taskkill /F /IM synergy-service.exe /T
taskkill /F /IM synergy-daemon.exe /T
taskkill /F /IM synergy-core.exe /T
taskkill /F /IM GoogleCrashHandler.exe /T
taskkill /F /IM GoogleCrashHandler64.exe /T
taskkill /F /IM GoogleDriveFS.exe /T
taskkill /F /IM GameBar.exe /Tg
taskkill /F /IM GameBarFTServer.exe /T
taskkill /F /IM Discord.exe /T
taskkill /F /IM Beeper.exe /T
taskkill /F /IM "Razer Central.exe" /T
taskkill /F /IM "Razer Synapse 3.exe" /T
taskkill /F /IM "RazerCentralService.exe" /T